...
@section('content')
    <div class="w-full p-5 flex items-center gap-3">
        ...
        <h3 class="text-xl font-semibold">Update Doctor</h3>
    </div>
    <div class="w-full flex justify-center">
        <div class="w-[500px] bg-white border border-gray-300 p-5 rounded-md shadow-lg">
            <form id="updateDoctorForm" method="post" action="{{ route('update.doctor', $doctor->id) }}" autocomplete="off">
                @csrf
                @method('PUT')
                ...
                <div class="w-full">
                    <fieldset class="fieldset">
                        ...
                        <select name="status" id="status" class="w-full select select-sm">
                            <option disabled selected>select a status</option>
                            <option value="available" {{ $doctor->doctor->status === 'available' ? 'selected' : '' }}>
                                Available
                            </option>
                            <option value="unavailable" {{ $doctor->doctor->status === 'unavailable' ? 'selected' : '' }}>
                                Unavailable
                            </option>
                        </select>
                    </fieldset>
                ...
                <div class="w-full">
                    <fieldset class="fieldset">
                        ...
                        <select name="specialty" id="specialty" class="w-full select select-sm">
                            ...
                            <option value="internal medicine - diabetes" {{ $doctor->doctor->specialty === 'internal medicine - diabetes' ? 'selected' : '' }}>Internal Medicine - Diabetes</option>
                            <option value="obstetrics - gynecology and ultra sound" {{ $doctor->doctor->specialty === 'obstetrics - gynecology and ultra sound' ? 'selected' : '' }}>
                                Obstetrics - Gynecology and
                                Ultra Sound</option>
                            <option value="internal medicine" {{ $doctor->doctor->specialty === 'internal medicine' ? 'selected' : '' }}>
                                Internal Medicine</option>
                            <option value="eent" {{ $doctor->doctor->specialty === 'eent' ? 'selected' : '' }}>EENT</option>
                            <option value="dental medicine" {{ $doctor->doctor->specialty === 'dental medicine' ? 'selected' : '' }}>Dental
                                Medicine</option>
                        </select>
                    </fieldset>
                </div>
                ...
                <button type="submit" id="updateBtn" class="btn btn-sm btn-block btn-primary mt-5">
                    <span id="buttonText">Update doctor</span>
